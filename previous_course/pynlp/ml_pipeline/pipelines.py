...
def naive_bayes_counts():
    return pipeline(preprocessing.std_prep(), representation.count_vectorizer({'min_df': 0.0,'ngram_range': (1,1)}), MultinomialNB())
...
def svm_libsvc_counts():
          	return pipeline(preprocessing.std_prep(), representation.count_vectorizer({'min_df': 1,'ngram_range': (1,2)}), svm.LinearSVC(max_iter=10000, dual=False, C=0.1))
